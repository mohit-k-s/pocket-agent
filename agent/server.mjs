...
server.listen(config.port, config.host, () => {
  ...
  console.log('Devin from iPhone is running:');
  for (const url of urls) console.log(`  ${url}`);
  console.log('Open localhost on your Mac to generate a pair code, then open a LAN URL on your phone.');
...
async function handleCreateTask(req, res, auth) {
  ...
  audit('task.created', { taskId, threadId, workspace: thread.workspace, executionMode: execution.mode, runnerUser: execution.runnerUser, by: auth.tokenMeta.id });
  runTask(taskId, threadId, prompt, thread.workspace, execution);
...
function runTask(taskId, threadId, prompt, workspace, execution) {
  ...
  const { command, args } = buildTaskCommand(prompt, execution);
...
function buildTaskCommand(prompt, execution) {
  const devinArgs = ['--permission-mode', config.devinPermissionMode, '-p', prompt];
